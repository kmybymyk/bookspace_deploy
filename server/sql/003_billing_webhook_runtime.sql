...
create table if not exists public.billing_webhook_events (
  ...
  normalized_plan public.bookspace_plan,
  normalized_status public.bookspace_subscription_status,
...
begin
  if not exists (
    select 1
    from pg_trigger
    where tgname = 'trg_billing_webhook_events_updated_at'
  ) then
...
begin
  if not exists (
    select 1 from pg_policies where schemaname = 'public' and tablename = 'billing_webhook_events' and policyname = 'bookspace_service_all_billing_webhook_events'
  ) then
    create policy bookspace_service_all_billing_webhook_events
